...
def parse_forecast(json_data) -> Forecast:
    ...
    for entry in json_data["properties"]["timeseries"]:
        ...
        _datetime : str = entry["time"]
        _datetime_parts = [p.strip() for p in _datetime.split("T")]
        ...
        timeseries.append(
            ForecastTimeStep(
                date=_datetime_parts[0],
                time=_datetime_parts[1].removesuffix("Z"),
                details=WeatherDetails(
